...
public class FileChannelTest {
	...
	/**
	 * 避免了用户态和内核态之间的数据拷贝，适用于相对大的文件映射
	 */
	public static void zeroCopy() {
		Path source = Paths.get(BASE_PATH, "readme.txt");
		Path dest = Paths.get(BASE_PATH, "replica.txt");
		try(FileChannel inChannel = FileChannel.open(source, StandardOpenOption.READ);
				FileChannel outChannel = FileChannel.open(dest, StandardOpenOption.CREATE, StandardOpenOption.READ, StandardOpenOption.WRITE)) {
			...
			byte[] content = new byte[inBuffer.limit()];
